function __fish_print_packages
	...
	# MacPorts.
	# This completes the package name from the ports tree.
	# True for installing new packages.
	# (strips version information)
	if type -f port >/dev/null
	   	port echo (commandline -tc)\* ^/dev/null| awk '{print $1}'
		return
	end
